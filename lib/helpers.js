...
function bySelfIncluding(specPath){
  return function(line){
    return resolve(specPath, path) === specPath;
  };
}
...
function verifyLineOrderingOf(spec){
  ...
  if(mixins.length){
    if(firstEl.command !== 'mixin')
      throw new errors.parsing.UnexpectedMixinDeclarationError(spec, mixins[0]);
    if(mixins.length > 1)
      throw new errors.parsing.MultipleMixinDeclarationsError(spec);
    if(quits.length)
      throw new errors.parsing.UnexpectedQuitInMixinError(spec, quits[0]);
    if(closes.length)
      throw new errors.parsing.UnexpectedCloseInMixinError(spec, closes[0]);
  }
}
