...
function readAndParse(spunFiles, argv){
  ...
  return function(waterfallCb){
    ...
    function includeResolver(line, lines, includedFiles){
      var arg;
      ...
      if(includedFiles.length > 1000) return stopQueue(new errors.InfiniteIncludeError(line, lines));

      line.includePath = includePath;

      queue.push(h.createReadContext(includePath, includedFiles), stopQueue);
    }
